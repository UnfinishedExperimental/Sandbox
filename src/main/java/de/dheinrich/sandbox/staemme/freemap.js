/*a89a8c03a3dbd5aeafe3740076105182*/
function FreeMap(div, coordToPixelScale, sectorSize, userHandler, bias) {
    ...
    this.coordByPixel = function(x, y, use_float) {
        if (use_float === true) {
            ...
            return[Math.floor(x / this.scale[0]), Math.floor(y / this.scale[1])]
    };
    ...
    this.coordByEvent = function(event) {
        var off = $(this.el.root).offset(), pos = [event.pageX - off.left + this.pos[0], event.pageY - off.top + this.pos[1]], x = Math.floor(pos[0] / this.scale[0]), y = Math.floor(pos[1] / this.scale[1]);
        return[x, y]
    };
    this.centerPos = function(x, y, fix_to_square) {
        var px = x * this.scale[0] - this.size[0] / 2 + this.scale[0] / 2, py = y * this.scale[1] - this.size[1] / 2 + this.scale[1] / 2;
    ...
    this.setPosPixel = function(x, y) {
        ...
        this.pos[0] = x;
        this.pos[1] = y;
        if (this.handler.onMovePixel)
            this.handler.onMovePixel(x, y);
        var new_sect_count = 0, posBR = [x + this.size[0], y + this.size[1]], sTL = this.sectorByPixel(x, y), sBR = this.sectorByPixel(posBR[0], posBR[1]);
        if (!compareCoord(this._lastTopLeftSector, sTL) || !compareCoord(this._lastBottomRightSector, sBR)) {
            var _sectors = [], load_sectors = [];
            for (var sx = sTL[0]; sx <= sBR[0]; sx++)
                for (var sy = sTL[1]; sy <= sBR[1]; sy++) {
                    var sectorID = sx + '_' + sy, sector = this._loadedSectors[sectorID];
                    if (!sector) {
                        sector = {id: sectorID, visible: false, loaded: true, sx: sx, sy: sy, x: sx * this.sectorSize, y: sy * this.sectorSize, _elements: [], _element_root: null, _map: _self_map, appendElement: function(el, x, y) {
                                ...
                                if (this.dom_fragment === undefined) {
                                    ...
                                    this.dom_fragment.appendChild(el)
                            }, spawn: function() {
                                if (this.visible)
                                    return;
                        ...
                        sector.spawn();
                        if (!this.handler.scrollBound || (sector.x >= (this.handler.scrollBound[0] - this.sectorSize) && sector.y >= (this.handler.scrollBound[1] - this.sectorSize) && sector.x < this.handler.scrollBound[2] && sector.y < this.handler.scrollBound[3]))
            ...
            if (load_sectors.length) {
                ...
                if (this.handler.loadSectors) {
                    ...
                    for (var idx = 0; idx < new_sect_count; idx++)
                        this.handler.loadSector(load_sectors[idx])
            }
            ;
            if (_sectors.length) {
                if (this.handler.preLoad)
                    this.handler.preLoad(_sectors.length);
                var _newVisibleSectors = {}, _len = _sectors.length;
                ...
                for (var idx in this._visibleSectors) {
                    if (!this._visibleSectors.hasOwnProperty(idx))
                        continue;
        ...
        if ((!this.lastCenterCoordPos || !compareCoord(coordPos, this.lastCenterCoordPos))) {
            if (this.handler.onMove)
                this.handler.onMove(coordPos[0], coordPos[1]);
    ...
    this.recalcViewport = function() {
        var x = this.pos[0], y = this.pos[1], coordTopLeft = this.coordByPixel(x, y), coordBottomRight = this.coordByPixel(x + this.size[0], y + this.size[1]);
    ...
    this.reload = function(set_pos, keep_data) {
        if (!keep_data) {
            for (var idx in this._loadedSectors) {
                if (!this._loadedSectors.hasOwnProperty(idx))
                    continue;
        ...
        ;
        this._visibleSectors = {};
        if (this.handler.onReload)
            this.handler.onReload();
        if (set_pos !== false) {
    ...
    this.resize = function(sx, sy, flags) {
        ...
        ;
        this.size = [sx, sy];
        if (this.handler.hasOwnProperty('onResize'))
            this.handler.onResize(sx, sy);
        if (flags & 1) {
            $(els[0]).animate({width: sx + 'px'}, {duration: 400, queue: false});
            $(els[1]).animate({height: sy + 'px'}, {duration: 400, queue: false})
        } else {
            $(els[0]).width(sx);
            $(els[1]).height(sy)
        }
    ...
    this.createResizer = function(min_size, max_size, step) {
        step = step || 1;
        $(this.el.root).resizable({grid: [step * this.scale[0], step * this.scale[1]], minWidth: min_size[0] * this.scale[0], maxWidth: max_size[0] * this.scale[0], minHeight: min_size[1] * this.scale[1], maxHeight: max_size[1] * this.scale[1], handles: 'se', zIndex: 13, start: $.proxy(function(event, ui) {
                if (this.handler.hasOwnProperty('onResizeBegin'))
                    this.handler.onResizeBegin();
                TWMap.busy = true;
                ...
                if (this.handler.hasOwnProperty('onResizeEnd'))
                    this.handler.onResizeEnd()
            }, this)}).resize($.proxy(function() {
            var el = $(this.el.root), sz = el.width() * 100000 + el.height();
            if (sz == this._lastResizeSize)
                return;
...
function FreeMapMover(map) {
    ...
    this.handleMouseUp = function(event) {
        if (event.changedTouches && event.changedTouches[0].identifier != this.touchIdentifier)
            return false;
    ...
    this.IEDragTimer = function() {
        var ev = {clientX: this.lastMousePositionForTimer[0], clientY: this.lastMousePositionForTimer[1]};
    ...
    this.handleMouseMove = function(event, is_timer_call) {
        ...
        if (this._map.handler.scrollBound) {
            var limit = this._map.handler.scrollBound, nTL = this._map.coordByPixel(pos[0], pos[1], true);
            if (nTL[0] < limit[0] && diff[0] > 0)
                diff[0] = 0;
            if (nTL[1] < limit[1] && diff[1] > 0)
                diff[1] = 0;
            var nBR = this._map.coordByPixel(pos[0] + this._map.size[0], pos[1] + this._map.size[1]);
            if (nBR[0] > limit[2] && diff[0] < 0)
                diff[0] = 0;
            if (nBR[1] > limit[3] && diff[1] < 0)
                diff[1] = 0
        }
        ;
        if ((diff[0] != 0 || diff[1] != 0) && this.moveDirty == false) {
            ...
            } else if (this.dragBeginHandler)
                this.dragBeginHandler();
            this.moveDirty = true
        }
        ;
        if (!this.allowDrag) {
            if (this.moveDirty && this.dragHandler)
                this.dragHandler(this.dragBeginPosition, this.mousePos, diff);
            return false
        }
        ;
