/*e6dfccf6d9c03a8e5ab37b7a5ae6d2d1*/
TWMap.initMap = function() {
    ...
    if (!$.browser.msie || true)
        map.createMover(this.mobile ? 1 : 2);
    this.map = map;
    ...
    ;
    this.popup.init();
    if (typeof (Warplanner) !== 'undefined')
        Warplanner.init()
};
TWMap.focus = function(x, y) {
    ...
    if (TWMap.map)
        TWMap.map.centerPos(x, y, true);
    TWMap.pos = [x, y]
};
TWMap.mapHandler.spawnSector = function(data, sector) {
    var beginX = sector.x - data.x, endX = beginX + TWMap.mapSubSectorSize, beginY = sector.y - data.y, endY = beginY + TWMap.mapSubSectorSize;
    if (TWMap.church.displayed || TWMap.politicalMap.displayed || TWMap.warMode)
    ...
    var x;
    for (x in data.tiles) {
        if (!data.tiles.hasOwnProperty(x))
            continue;
        x = parseInt(x);
        if (x < beginX || x >= endX)
            continue;
        ...
        for (y in data.tiles[x]) {
            if (!data.tiles[x].hasOwnProperty(y))
                continue;
            y = parseInt(y);
            if (y < beginY || y >= endY)
                continue;
            ...
            if (v) {
                var owner = v.owner, ally = (v.owner > 0 && TWMap.players[v.owner]) ? TWMap.players[v.owner].ally : 0;
                if (v.owner == 0) {
                    if (TWMap.villageGroups[v.id]) {
                        var col = TWMap.villageGroups[v.id], circle = TWMap.createVillageDot(col);
                    ...
                    if (v.id == game_data.village.id) {
                        ...
                        col = TWMap.getColorByPlayer(owner, ally, v.id);
...
TWMap.scrollBlock = function(x, y) {
    if (this.scrolling)
        return;
    this.scrolling = true;
    var dst = [TWMap.pos[0] + TWMap.size[0] * x, TWMap.pos[1] + TWMap.size[1] * y], scrollTID = setInterval(function() {
        ...
        if (TWMap.pos[0] != dst[0])
            p[0] += x;
        if (TWMap.pos[1] != dst[1])
            p[1] += y;
        TWMap.focus(p[0], p[1])
    }, 30)
};
...
TWMap.resizeMinimap = function(size, smooth) {
    var dstWidth = size * this.minimap.scale[0], dstHeight = size * this.minimap.scale[1];
...
TWMap.mapHandler.onResize = function(sx, sy) {
    ...
    if (!TWMap.isDragResizing)
        TWMap.notifyMapSize(TWMap.isAutoSize)
};
