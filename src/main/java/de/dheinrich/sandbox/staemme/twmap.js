/*183130701f8f584fb6e9d2028a871739*/
var TWMap = {map: null, minimap: null, mobile: false, fullscreen: false, cachePopupContents: false, mapSubSectorSize: 5, strings: {}, urls: {}, colors: {}, graphics: null, image_base: null, images: ['gras1.png', 'gras2.png', 'gras3.png', 'gras4.png', 'v1_left.png', 'v1.png', 'v2_left.png', 'v2.png', 'v3_left.png', 'v3.png', 'v4_left.png', 'v4.png', 'v5_left.png', 'v5.png', 'v6_left.png', 'v6.png', 'b1_left.png', 'b1.png', 'b2_left.png', 'b2.png', 'b3_left.png', 'b3.png', 'b4_left.png', 'b4.png', 'b5_left.png', 'b5.png', 'b6_left.png', 'b6.png', 'berg1.png', 'berg2.png', 'berg3.png', 'berg4.png', 'forest0000.png', 'forest0001.png', 'forest0010.png', 'forest0011.png', 'forest0100.png', 'forest0101.png', 'forest0110.png', 'forest0111.png', 'forest1000.png', 'forest1001.png', 'forest1010.png', 'forest1011.png', 'forest1100.png', 'forest1101.png', 'forest1110.png', 'forest1111.png', 'see.png', 'event_xmas.png', 'event_easter.png'], images_secret_blue: [], images_secret_yellow: [], scriptMode: false, warMode: false, warModeGeneration: 0, villages: {}, villageKey: {}, players: {}, allies: {}, playerGroups: {}, allyGroups: {}, villageGroups: {}, villageIcons: {}, commandIcons: {}, allyRelations: {}, reservations: {}, targets: [], pos: [], size: [0, 0], isAutoSize: false, minimap_offset: [0, 0], minimap_size: [0, 0], currentCon: null, currentVillage: null, scrollBound: [0, 0, 1000, 1000], goFullscreen: function() {
        if (!TWMap.premium)
            return false;
        var el = document.getElementById('map_wrap'), events = 'fullscreenchange mozfullscreenchange webkitfullscreenchange';
        ...
        $(document).bind(events, function() {
            ...
            $('#map_coord_y_wrap').detach().appendTo('#map_wrap');
            $('#map_coord_x_wrap').detach().appendTo('#map_wrap');
            $('#map_popup').detach().appendTo('#map_wrap');
            $('#minimap').detach().appendTo('#map_wrap');
            $('#fullscreen').hide();
            $(document).unbind(events).bind(events, function() {
                $('#map_coord_y_wrap').detach().appendTo($('#map_coord_y_container'));
                $('#map_coord_x_wrap').detach().appendTo($('#map_coord_x_container'));
        ...
        for (i = 0; i < data_array.length; i++) {
            var inf = data_array[i], data = inf.data, sx = data.x, sy = data.y;
            TWMap.storeVillage.set(sx, sy, inf.data);
            if (inf.tiles)
                TWMap.storeTiles.set(sx, sy, inf.tiles);
            if (inf.pmap)
                TWMap.storePolitical.set(sx, sy, inf.pmap);
            inf = null;
            data = null
        }
    }, mapHandler: {_waitingSectors: {}, onReceiveSectorInformation: function(data_array, skip_store) {
            if (!skip_store)
                TWMap.storeSectorInformation(data_array);
            for (var sector_idx = 0; sector_idx < data_array.length; sector_idx++) {
                var inf = data_array[sector_idx], data = inf.data, sx = data.x, sy = data.y, sectorData = this._waitingSectors[sx + '_' + sy];
                if (!sectorData)
                    continue;
                if (inf.tiles)
                    sectorData.tiles = inf.tiles;
                if (inf.pmap)
                    sectorData.pmap = inf.pmap;
                sectorData.x = sx;
                sectorData.y = sy;
                sectorData.data = data;
                var village_ids = [];
                for (var x in data.villages) {
                    if (!data.villages.hasOwnProperty(x))
                        continue;
                    x = parseInt(x);
                    for (var y in data.villages[x]) {
                        if (!data.villages[x].hasOwnProperty(y))
                            continue;
                        y = parseInt(y);
                        var v = data.villages[x][y];
                        if (v[2] === 0)
                            v[2] = TWMap.strings.barbarianVillage;
                        var xy = (sx + x) * 1000 + sy + y;
                        TWMap.villageKey[v[0]] = xy;
                        TWMap.villages[xy] = {id: v[0], img: v[1], name: v[2], points: v[3], owner: v[4], mood: v[5], bonus: v[6], xy: xy};
                ...
                for (var player_id in data.players) {
                    ...
                    TWMap.players[player_id] = {name: v[0], points: v[1], ally: v[2], newbie: v[3], sleep: v[4]}
                }
                ;
                for (var ally_id in data.allies) {
                    if (!data.allies.hasOwnProperty(ally_id))
                        continue;
                    var v = data.allies[ally_id];
                    TWMap.allies[ally_id] = {name: v[0], points: v[1], tag: v[2]}
                }
                ;
            ...
            for (var i = 0; i < sectors.length; i++) {
                var sector = sectors[i], sx = sector.x - (sector.x % 20), sy = sector.y - (sector.y % 20), sector_id = sx + '_' + sy, wait_data = this._waitingSectors[sector_id];
                if (wait_data) {
                    wait_data.queue.push(sector)
                } else {
                    wait_data = {id: sector_id, x: sx, y: sy, tiles: null, pmap: null, data: null, queue: [sector]};
                    this._waitingSectors[sector_id] = wait_data;
                    wait_sectors.push(wait_data)
                }
            }
            ;
            if (wait_sectors.length < 1)
                return;
            ...
            for (var i = 0; i < wait_sectors.length; i++) {
                ...
                if (wait_data.data !== null && wait_data.tiles !== null && wait_data.pmap !== null) {
                    ...
                    this._sector_request_queue.push(wait_data)
            }
            ;
            if (this._sector_request_queue.length > 0) {
                ...
                for (var i = 0; i < this._sector_request_queue.length; i++) {
                    var wait_data = this._sector_request_queue[i], query_val = 0;
                    if (wait_data.tiles === null)
                        query_val += 1;
                    if (wait_data.pmap === null)
                        query_val += 2;
            ...
            for (var cy = Math.max(0, y - 20); cy < cymax; cy++) {
                if (TWMap._coord_el_y_active[cy])
                    continue;
            ...
            for (var cx = Math.max(0, x - 20); cx < cxmax; cx++) {
                if (TWMap._coord_el_x_active[cx])
                    continue;
            ...
            if (village) {
                if (TWMap.warMode && Warplanner.admin) {
                    ...
                    if (!e || ($.browser.msie && ~~($.browser.version) < 8))
            ...
            if (isConBorder) {
                ...
                el.className = 'map_border';
            if (TWMap.night && !TWMap.classic_gfx)
                el.className += '_night';
            el.style.zIndex = '3';
        ...
        }}, minimapHandler: {loadSector: function(sector) {
            var el = document.createElement('img');
            el.style.position = 'absolute';
            el.style.zIndex = '1';
            var vid = '';
            if (game_data && game_data.village)
                vid = '&village_id=' + game_data.village.id;
            var pmap = (TWMap.politicalMap.displayed && !TWMap.warMode) ? TWMap.politicalMap.filter : 0, church = TWMap.church.displayed ? 1 : 0, war = TWMap.warMode ? 1 + TWMap.warModeGeneration : 0;
            ...
            for (var village_id in TWMap.secrets) {
                if (!TWMap.secrets.hasOwnProperty(village_id))
                    continue;
                var type = TWMap.secrets[village_id][0], x = TWMap.secrets[village_id][1] - sector.x, y = TWMap.secrets[village_id][2] - sector.y;
                if (x < 0 || y < 0 || x >= TWMap.minimap.sectorSize || y >= TWMap.minimap.sectorSize)
                    continue;
            ...
            TWMap.busy = true;
            if (TWMap.map) {
                var rx = (x / TWMap.minimap.scale[0]) + TWMap.minimap_offset[0], ry = (y / TWMap.minimap.scale[1]) + TWMap.minimap_offset[1];
        ...
        if (TWMap.warMode && Warplanner.data[v.id] && Warplanner.data[v.id].type !== 'D') {
            var el_img = document.createElement('img'), img = (Warplanner.data[v.id].type === 'A') ? 'attack' : 'fake';
            el_img.style.position = 'absolute';
        ...
        this.politicalMap.optionToggle = new MapToggleBox({id: 'pmap_options'});
        this.centerToggle = new MapToggleBox({id: 'centercoords', url: this.urls.centerCoords});
        ...
        this.initMap();
        if (TWMap.premium && !TWMap.light && (document.body.requestFullScreen || document.body.mozRequestFullScreen || document.body.webkitRequestFullScreen))
            $('#fullscreen').show();
        ...
        for (var res in TWMap.reservations) {
            if (!TWMap.reservations.hasOwnProperty(res))
                continue;
            if (!TWMap.villageIcons[res])
                TWMap.villageIcons[res] = [];
            var img = '/graphic/map/reserved_' + TWMap.reservations[res] + '.png';
            TWMap.villageIcons[res].unshift({img: img})
        }
        ;
        this.context.init();
        var coord = window.location.hash.match(/^#([0-9]+);([0-9]+)$/);
        if (TWMap.map && coord !== null)
            setTimeout(function() {
        ...
        var mapsize = this.size.join('x'), minisize = this.minimap_size.join('x'), cacheval = mapsize + '-' + minisize;
        if (TWMap._lastNotifiedMapsize == cacheval)
            return;
        ...
        if (!mobile && !TWMap.fullscreen) {
            $.ajax({url: this.urls.sizeSave, data: 'map_size=' + mapsize + '&minimap_size=' + minisize, type: 'GET', success: function() {
                    if (doRefresh)
                        window.location.reload()
                }})
        } else if (isAuto) {
            $.cookie("mobile_mapsize", 0, {expires: 7})
        } else
            $.cookie("mobile_mapsize", mapsize, {expires: 7})
    }, scaleMinimap: function() {
        var sz_min = [~~(this.minimap.size[0] / this.minimap.scale[0]), ~~(this.minimap.size[1] / this.minimap.scale[1])], sz_max = [~~(this.map.size[0] / this.map.scale[0]), ~~(this.map.size[1] / this.map.scale[1])], offset = [~~((sz_min[0] - sz_max[0]) / 2), ~~((sz_min[1] - sz_max[1]) / 2)];
        this.minimap_offset = offset;
        this.minimap_size = sz_min;
        var el = document.getElementById('minimap_viewport'), vp_width = sz_max[0] * this.minimap.scale[0], vp_height = sz_max[1] * this.minimap.scale[1];
        el.style.width = vp_width + 'px';
        el.style.height = vp_height + 'px';
        var vp_left = offset[0] * this.minimap.scale[0], vp_top = offset[1] * this.minimap.scale[1];
        ...
        UI.SuccessMessage(TWMap.strings.changesSaved)
    }, centerList: {enabled: false, el_x: null, el_y: null, last_x: 0, last_y: 0, init: function() {
            if (!this.enabled)
                return;
            ...
            var x = TWMap.pos[0], y = TWMap.pos[1];
            if (!this.el_x || !this.el_y)
                return;
            if (this.el_x.is(':focus') || this.el_y.is(':focus'))
                return;
            if ((this.last_x && this.el_x.val() != this.last_x) || (this.last_y && this.el_y.val() != this.last_y))
                return;
            this.last_x = x;
            this.last_y = y;
            this.el_x.val(x);
            ...
            var el = $('#center_' + id), n = $('#centerlist_new_tpl').clone().attr('id', 'centeredit_' + id);
            n.data('el', el).data('id', id).show();
            var el_x = n.find('input[name=center_x]').val(el.data('x')), el_y = n.find('input[name=center_y]').val(el.data('y')), el_name = n.find('input[name=center_name]').val(this.getName(id));
            el.after(n).detach();
            el_x.focus()
        }, del: function(id) {
            var handleDelete = function() {
                TWMap.centerList.save(id, 0, 0, '', true)
            }, name = this.getName(id), msg = TWMap.strings.confirmCenterDelete.replace(/%name%/, name), buttons = [{text: "OK", callback: handleDelete, confirm: true}];
            UI.ConfirmationBox(msg, buttons)
        }, submit: function(t) {
            t = $(t).parent();
            t.find('input[type=submit]').attr('disabled', 'disabled');
            var el = t.data('el'), id = t.data('id'), x = t.find('input[name=center_x]').val(), y = t.find('input[name=center_y]').val(), name = t.find('input[name=center_name]').val();
            if (el && id) {
                t.before(el).remove();
                ...
                this.save(-1, x, y, name, true);
            return false
        }, save: function(id, x, y, name, reload) {
            $.post(TWMap.urls.centerSave, {id: id, x: x, y: y, name: name}, function(data) {
                if (reload)
                    $('#centercoords').html(data)
            }, 'html')
        }, getName: function(id) {
        ...
        return[~~(xy / 1000), xy % 1000]
    }, popup: {enabled: true, optionToggle: null, attackDots: ['', '/dots/green.png', '/dots/yellow.png', '/dots/red.png', '/dots/blue.png', '/dots/red_yellow.png', '/dots/red_blue.png'], attackMaxLoot: ['/max_loot/0.png', '/max_loot/1.png'], _px: 0, _py: 0, init: function() {
            var cookie = $.cookie('hide_map_popup');
            if (cookie === 'yes') {
                this.enabled = false
            } else
                $('#show_popup').attr('checked', 'checked');
            this.optionToggle = new MapToggleBox({id: 'popup_options'});
            ...
            if (data[0]) {
                for (var k = 0; data[k] !== undefined; k++)
                    this.receivedPopupInformationForSingleVillage(data[k])
            } else {
            ...
            if (!TWMap.cachePopupContents)
                delete this._cache[data.id]
        }, popupOptionsSet: function() {
            var checkboxes = $('#popup_options').find('input[type=checkbox]'), state = false;
            ...
            if (TWMap.context._curFocus == -1)
                return true;
            var p = TWMap.CoordByXY(TWMap.context._curFocus), diff = [Math.abs(x - p[0]), Math.abs(y - p[1])];
            return diff[0] >= 2 || diff[1] >= 2
        }, loadVillage: function(village_id) {
            $.ajax({url: TWMap.urls.villagePopup.replace(/__village__/, village_id), dataType: 'json', type: 'GET', success: function(data) {
            ...
            if (this != TWMap.popup)
                return false;
            var pos = TWMap.map.coordByEvent(event), x = pos[0], y = pos[1], coordidx = x * 1000 + y, village = TWMap.villages[coordidx];
            if (village && TWMap.map.inViewport(x, y) && this._isAwayFromContext(x, y)) {
                ...
                if (TWMap.map.el.mover)
                    TWMap.map.el.mover.style.cursor = 'pointer';
                if (!this.enabled)
                    return false;
            ...
            if (this._currentVillage != village.id)
                return;
            var owner = TWMap.players[village.owner], dat = {bonus: null, name: village.name, x: x, y: y, continent: TWMap.con.continentByXY(x, y), points: village.points, owner: null, newbie: null, ally: null, extra: null, units: [], units_display: {}};
            if (village.bonus)
                dat.bonus = {text: village.bonus[0], img: TWMap.image_base + village.bonus[1]};
            if (owner) {
                dat.owner = owner;
            ...
            if (this.extraInfo && TWMap.popup.popupOptionsSet()) {
                ...
                if (typeof extra === 'undefined') {
                    ...
                    var unit_checked = {total: $('#map_popup_units').is(":checked"), home: $('#map_popup_units_home').is(":checked"), time: $('#map_popup_units_times').is(":checked")};
                    ...
                    if (unit_checked.total || unit_checked.home || unit_checked.time)
                        for (var name in extra.units) {
                            if (!extra.units.hasOwnProperty(name))
                                continue;
                            var total = parseInt(extra.units[name]['count']['home']) + parseInt(extra.units[name]['count']['foreign']), has_total = (unit_checked.total && total != 0);
                            if (has_total || (unit_checked.time && extra.units[name]['time'])) {
                                ...
                                ;
                                dat.units.push({name: name, image: extra.units[name]['image'], time: extra.units[name]['time'], count: cntstr})
                            }
                        }
                    ;
                    dat.extra = extra
            ...
            var el_size = [this.el.width(), this.el.height()], constraint = [$(window).scrollLeft() + 3, $(window).scrollTop() + 3, $(window).scrollLeft() + $(window).width() - 3, $(window).scrollTop() + $(window).height() - 3];
            ...
            if (TWMap.map.el.mover)
                TWMap.map.el.mover.style.cursor = 'move';
            this.hide()
        }, hide: function() {
            if (this._is_visible) {
        ...
        if (TWMap.light) {
    ...
    }, politicalMap: {displayed: false, filter: 1, optionToggle: null, toggle: function(toggle_options) {
            var displayed = $('#politicalmap').is(':checked'), filter = ~~$('#pmap_options').find('input[name="pmap_filter"]:checked').val();
            if ($('#pmap_show_topo').is(':checked'))
                filter += 8;
            if ($('#pmap_show_map').is(':checked'))
                filter += 16;
            $.ajax({url: TWMap.urls.changeShowPolitical, data: {topo_show_politicalmap: displayed, topo_politicalmap_filter: filter}, type: 'GET', success: function() {
                    TWMap.notifySavedChanges()
                }});
            if (toggle_options)
                if (displayed) {
                    ...
                    this.optionToggle.hide();
            if (displayed != this.displayed || (displayed && filter != this.filter)) {
                this.displayed = displayed;
                this.filter = filter;
                TWMap.reload()
            }
        }}, church: {displayed: false, enabled: ($.browser.msie != true), toggle: function() {
            this.displayed = $('#belief_radius').is(':checked');
            $.ajax({url: TWMap.urls.changeShowBelief, data: {topo_show_belief: this.displayed}, type: 'GET', success: function() {
                    TWMap.notifySavedChanges()
                }});
            TWMap.reload()
        }}, con: {SEC_COUNT: 1, SUB_COUNT: 1, CON_COUNT: 1, continentByXY: function(x, y) {
            var con_x = Math.floor(x / (TWMap.con.SEC_COUNT * TWMap.con.SUB_COUNT)), con_y = Math.floor(y / (TWMap.con.SEC_COUNT * TWMap.con.SUB_COUNT));
            return con_x + con_y * TWMap.con.CON_COUNT
        }}, context: {_curFocus: -1, _visible: true, strings: {}, _circlePos: [[-12, -12], [-12, -49], [20, -30], [20, 6], [-11, 25], [-44, 6], [-44, -30], [20, -30], [20, 6]], _otherOrder: ['mp_info', 'mp_lock', 'mp_profile', 'mp_msg', 'mp_fav', 'mp_res', 'mp_att', 'mp_farm_a', 'mp_farm_b'], _ownOrder: ['mp_info', 'mp_recruit', 'mp_profile', 'mp_overview', 'mp_fav', 'mp_res', 'mp_att', 'mp_farm_a', 'mp_farm_b'], _showPremium: false, init: function() {
            ...
            $.ajax({url: TWMap.urls.changeUseContext, data: {use_context_menu: this.enabled}, type: 'GET', success: function() {
                    TWMap.notifySavedChanges()
                }})
        }, FATooltip: {init: function(button, x, y) {
                var $button = $('#' + button), minspeed = $button.data('minspeed'), startVillage = game_data.village.coord.split("|"), distance = this.distance(startVillage[0], startVillage[1], x, y), duration = this.calculateDuration(distance, minspeed);
                $button.data("duration", duration);
                this.bind($button)
            }, bind: function($button) {
                $button.bind('mouseover', function(e) {
                    if (typeof ($button.data("tooltip")) == 'undefined') {
                        UI.ToolTip($button, {bodyHandler: TWMap.context.FATooltip.create});
                        $button.triggerHandler('mouseover')
                    }
                })
            }, create: function() {
                var $button = $(this), tpl = $button.data("tooltip-tpl"), duration = $button.data("duration");
                ...
                var hours = Math.floor(duration / 3600), mins = Math.floor(duration / 60) % 60;
                if (mins < 10)
                    mins = "0" + mins;
                var secs = duration % 60;
                if (secs < 10)
                    secs = "0" + secs;
                var durationString = hours + ":" + mins + ":" + secs, html = "<span style='line-height: 20px'>" + durationString + "</span><br />";
                return html
            ...
            if (TWMap.light) {
                ...
                TWMap.popup.hide();
                var pos_center = TWMap.map.pixelByCoord(x, y), pos_topleft = TWMap.map.pos;
                pos = [pos_center[0] - pos_topleft[0], pos_center[1] - pos_topleft[1]]
            }
            ;
            pos[0] += TWMap.tileSize[0] / 2;
            pos[1] += TWMap.tileSize[1] / 2;
            var me = this, button_order = (village.owner == game_data.player.id) ? this._ownOrder : this._otherOrder, buttons = [], circle_pos = this._circlePos;
            $(button_order).each(function(k, v) {
                if (village.owner == '0' && (v == 'mp_profile' || v == 'mp_msg')) {
                    ...
                    return
                } else if (game_data.player.ally_id == 0 && v == 'mp_lock') {
                    return
                } else if (!village.points && (v == 'mp_fav' || v == 'mp_lock'))
                    return;
                if (v == 'mp_lock' && TWMap.reservations[village.id])
                    v = 'mp_unlock';
                if (v == 'mp_fav' && jQuery.inArray(parseInt(village.id), TWMap.targets) != -1)
                    v = 'mp_unfav';
                if (v == 'mp_farm_a' || v == 'mp_farm_b')
                    TWMap.context.FATooltip.init(v, x, y);
            ...
            $('#' + v).unbind('click').click(function(e) {
                ...
                if (this._last && (now - this._last < 900))
                    return;
                this._last = now;
            ...
            switch (key) {
                ...
                case'mp_unlock':
                    ...
                    ;
                    if (data.notice)
                        UI.InfoMessage(data.notice, null, null, message_container);
                    if (!TWMap.villageIcons[data.village])
                        TWMap.villageIcons[data.village] = [];
                    ...
                    if (data.type == 'delete') {
                        ...
                        icons.unshift({img: '/graphic/map/reserved_player.png'})
                    }
                    ;
                    TWMap.popup.invalidateCache();
                    TWMap.reload();
                    ...
                    if (data.error) {
                        ...
                        if (TWMap.premium) {
                            if (!TWMap.commandIcons[data.target_village]) {
                                TWMap.commandIcons[data.target_village] = [{img: 'attack'}]
                            } else {
                                var attack = false;
                                ...
                                if (!attack)
                                    TWMap.commandIcons[data.target_village].push({img: 'attack'})
                            }
                            ;
                            TWMap.reload()
                        }
...
function TWMapStore(_size, _ttl) {
    var that = this, size = _size, ttl = _ttl * 1000, data = new Array(size), i, j;
    for (i = 0; i < size; i++) {
    ...
    this.get = function(x, y) {
        var dx = (x / 20) % size, dy = (y / 20) % size, e = data[dx][dy], now = new Date().getTime();
        if (e === null || e[0] !== x || e[1] !== y)
            return null;
        if (e[2] < now) {
            data[dx][dy] = null;
            return null
    ...
    this.set = function(x, y, obj) {
        var dx = (x / 20) % size, dy = (y / 20) % size, expire = new Date().getTime() + ttl, el = [x, y, expire, obj];
        data[dx][dy] = el
    }
}