...
var MapCanvas = {box: 3, init: function() {
        var i;
        if (this.churchData)
            for (i = 0; i < this.churchData.length; i++)
                this.churchData[i][2] *= this.churchData[i][2]
    }, createCanvas: function(sector, data) {
        var canvas = document.createElement('canvas');
        if (!canvas || !canvas.getContext)
            return null;
        var tileScale = TWMap.map.scale, size = TWMap.map.sectorSize;
        ...
        ctx.save();
        var churches = this.churchData, map = data.pmap[0], ally = data.pmap[1], width = 20;
        if (map.length < 1)
            map = null;
        var tilew = canvas.offsetWidth / size, tileh = canvas.offsetHeight / size;
        ctx.scale(tilew / 37.75, tileh / 37.75);
        var x, y, cells, pmap_filter = TWMap.politicalMap.filter % 8, war_mode = TWMap.warMode, show_pmap = !war_mode && TWMap.politicalMap.displayed && (TWMap.politicalMap.filter & 16), war;
        if (war_mode)
            war = Warplanner.data;
        for (y = sector.y - 1; y < sector.y + size + 1; y++)
            for (x = sector.x - 1; x < sector.x + size + 1; x++) {
                ...
                ;
                if (show_pmap) {
                    var dx = x - data.x + 1, dy = y - data.y + 1, ee = dx + dy * (width + 2), show_cell;
                    if (map && map[ee] !== 0) {
                        var aa = (dx - 1) + (dy - 1) * (width + 2), bb = (dx - 0) + (dy - 1) * (width + 2), cc = (dx + 1) + (dy - 1) * (width + 2), dd = (dx - 1) + (dy) * (width + 2), ff = (dx + 1) + (dy) * (width + 2), gg = (dx - 1) + (dy + 1) * (width + 2), hh = (dx - 0) + (dy + 1) * (width + 2), ii = (dx + 1) + (dy + 1) * (width + 2), we = dx < 1, ea = dx >= width + 1, no = dy < 1, so = dy >= width + 1;
                        if (pmap_filter == 1 || map[ee] == game_data.player.id) {
                            ...
                            var col = [255, 0, 255], pid = map[ee], aid = ally[ee];
                ...
                if (TWMap.church.displayed && churches) {
                    ...
                    for (i = 0; i < churches.length; i++) {
                        var cx = churches[i][0], cy = churches[i][1], crad = churches[i][2];
        ...
        if (points.length < 1)
            return;
        var p = 2, normals = [], x1, y1, x2, y2, dx, dy, sqlen, len, nx, ny, nx1, ny1, nx2, ny2, nx0, ny0, cg;
        ...
        ;
        var oldgco = ctx.globalCompositeOperation, oldfs = ctx.fillStyle;
        if (!grad)
            ctx.fillStyle = "rgba(" + color[0] + "," + color[1] + "," + color[2] + ",.7)";
