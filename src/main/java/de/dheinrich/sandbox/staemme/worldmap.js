...
var Worldmap = {Data: {t: 0}, init: function(t) {
        Worldmap.Data.t = t;
        $("#worldmap").draggable({stop: Worldmap.savePosition, containment: [0, 60]})
    }, toggle: function() {
        if ((typeof toggle_value == 'undefined') || (toggle_value == false)) {
            if (typeof toggle_value == 'undefined')
                Worldmap.reload();
            toggle_value = true
        } else
            toggle_value = false;
        ...
        if (Worldmap.Data.t > 0)
            params = params + '&t=' + Worldmap.Data.t;
        ...
        img.onload = function() {
            ...
            var x_bias = 500 - this.width / 2, y_bias = 500 - this.height / 2;
            $('#worldmap_image > input').width(this.width).height(this.height).click(function(e) {
                var off = $(this).offset(), x = e.offsetX ? e.offsetX : (e.pageX - this.offsetLeft - off.left), y = e.offsetY ? e.offsetY : (e.pageY - this.offsetTop - off.top);
                x += x_bias;
                y += y_bias;
                Worldmap.toggle();
